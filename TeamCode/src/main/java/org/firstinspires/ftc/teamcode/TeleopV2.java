...
/**
 * Simplified TeleOp Mode for FTC Robot with integrated Color Sensor.
 *
 * Features:
 * - Basic Mecanum Drive
 ...
 * - State Management
 *
 * --- NEW FEATURE (Color Sensor) ---
 * - When Gamepad2 Right Bumper is pressed, the color sensor thread is activated.
 * - When Gamepad2 Left Bumper is pressed, the color sensor thread is deactivated.
 * - When active, if the sensor reads red or blue, the claw will automatically close.
 */
...
public class TeleopV2 extends OpMode {
    ...
    // -----------------------
    // Color Sensor Components (NEW)
    // -----------------------
    private ColorSensor colorSensor;
    private Thread colorSensorThread = null;
    private volatile boolean colorSensorActive = false;
    private float[] hsvValues = new float[3];
    // For edge detection for sensor activation/deactivation
    private boolean prevGamepad2RightBumperForSensor = false;
    private boolean prevGamepad2LeftBumperForSensor = false;
    ...
    private static final double ROTATE_SCORE   = 0.22;
    private static final double ROTATE_WALL    = 0.22;
    private static final double ROTATE_PRE    = 0.22; //.78 IS FLIPPED
    ...
    // -----------------------
    private boolean previousGamepad2LeftBumper = false;  // For scoring sequence edge detection
    private boolean previousGamepad2RightTrigger = false;  // For grabbing sequence edge detection
    ...
    public void init() {
        ...
        // Initialize Color Sensor (do not start sensor thread yet)
        colorSensor = hardwareMap.get(ColorSensor.class, "colorSensor");

        // Set Initial Servo Positions (if desired, uncomment and adjust)
        // setServoPositions(ARM_READY, CLAW_OPEN, WRIST_READY, ROTATE_READY);

        telemetry.addData("Status", "Initialized");
        telemetry.addData("Color Sensor", "Inactive");
        telemetry.update();
    }
    ...
    // -----------------------
    // Color Sensor Thread Methods (NEW)
    // -----------------------
    private void startColorSensorThread() {
        colorSensorActive = true;
        colorSensorThread = new Thread(new Runnable() {
            @Override
            public void run() {
                while (colorSensorActive) {
                    int r = colorSensor.red();
                    int g = colorSensor.green();
                    int b = colorSensor.blue();

                    // Multiply sensor values by 255 for conversion if needed.
                    Color.RGBToHSV(r * 255, g * 255, b * 255, hsvValues);
                    float hue = hsvValues[0];

                    // Check for red (hue < 30 or > 330) or blue (approx. 150-270)
                    if ((hue < 30 || hue > 330) || (hue > 150 && hue < 270)) {
                        // Automatically close the claw
    ...
    @Override
    public void loop() {
        // --- Color Sensor Activation/Deactivation (NEW) ---
        // Press Gamepad2 Right Bumper to activate sensor (edge detection)
        if (gamepad2.right_bumper && !prevGamepad2RightBumperForSensor && !colorSensorActive) {
            startColorSensorThread();
            telemetry.addData("Color Sensor", "Activated");
        }
        // Press Gamepad2 Left Bumper to deactivate sensor (edge detection)
        if (gamepad2.left_bumper && !prevGamepad2LeftBumperForSensor && colorSensorActive) {
            stopColorSensorThread();
            telemetry.addData("Color Sensor", "Deactivated");
        }
        prevGamepad2RightBumperForSensor = gamepad2.right_bumper;
        prevGamepad2LeftBumperForSensor = gamepad2.left_bumper;

        // -----------------------
        // Existing TeleOp Controls
        // -----------------------
        handleDrive();

        // Manual controls are enabled only when:
        // - No automated sequence is running,
        // - The pivot is fully down,
        ...
        // Check for wall sequence delay (example: 1000ms) before starting pivot up.
    ...
    private void sendTelemetry() {
        ...
        telemetry.addData("Color Sensor Active", colorSensorActive);
        telemetry.update();
    }
}